...
class Unicorn_compound7_longconv_2(nn.Module):
    def __init__(self, num_class):
        super(Unicorn_compound7_longconv_2, self).__init__()
        ...
        self.head = nn.Sequential(
            MBblock(64, 64, 5, 2, 1, None),
            MBblock(64, 64, 5, 1, 2, None),
            MLblock(64, 64, 3, 1, 2, None)
            )
        self.seq1 = nn.Sequential(
            MBblock(64, 80, 5, 2, 4, None),
            MBblock(80, 80, 5, 1, 3, None),
            MBblock(80, 80, 3, 1, 3, None),
            MBblock(80, 80, 3, 1, 3, None),
            MBblock(80, 80, 5, 1, 3, None)
            )
        self.seq2 = nn.Sequential(
            MBblock(80, 160, 5, 2, 4, None),
        ...
        self.seq3 = nn.Sequential(
            MBblock(160, 320, 5, 2, 4, None),
            MBblock(320, 320, 5, 1, 3, None),
            MBblock(320, 320, 3, 1, 3, SeModule(320, 3)),
            MBblock(320, 320, 3, 1, 3, SeModule(320, 3))
            )

        self.tail = nn.Sequential(
            nn.Conv2d(320, 320, kernel_size=8, stride=1, groups=320),
            nn.BatchNorm2d(320),
            nn.PReLU(),
            nn.Conv2d(320, num_class, kernel_size=1, stride=1, bias=False)
        )
        self.ratio = nn.Sequential(
            nn.AdaptiveMaxPool2d(1),
            nn.Conv2d(320, 160, kernel_size=1, stride=1),
            nn.BatchNorm2d(160),
            nn.PReLU(),
            nn.Conv2d(160, 160, kernel_size=1, stride=1),
            nn.BatchNorm2d(160),
            nn.PReLU(),
            nn.Conv2d(160, num_class, kernel_size=1, stride=1, bias=False)
        )
        self.residual_0 = Re_attention1(64, 64, 2)
        self.residual_1 = Re_attention1(64, 80, 2)
        self.residual_2 = Re_attention1(80, 160, 2)
        self.residual_3 = Re_attention1(160, 320, 2)
    def forward(self, x):
        x = self.conv1(x)
        y = self.residual_0(x)
        x = self.head(x)
...
if __name__=="__main__":
    ...
    model = Unicorn_compound7_longconv_2(128)
    torch.save(model.state_dict(), '1.pth')
    semode = SeModule(64, 2)
    x = torch.randn(2, 1, 128, 128)
    # torch.load_state_dict(torch.load('./checkpoints/'))
    y = model(x)
    print(y.shape)
