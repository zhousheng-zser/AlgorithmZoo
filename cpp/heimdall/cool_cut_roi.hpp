...
namespace glasssix
{
	namespace heimdall
	{
		struct cordinate_roi {
			...
			std::vector<std::vector<cv::Point2f>> origin_cordinate;
		...
		private:
			...
			std::pair<std::vector<std::vector<cv::Point2f>>, std::vector<std::vector<cv::Point2f>>> get_coordinate_order(std::vector<std::vector<cv::Point2f>>& cordi, cv::Mat& img, const int& threshold);
