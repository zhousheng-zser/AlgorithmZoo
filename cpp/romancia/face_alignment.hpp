...
namespace glasssix::exposing::impl
{
	template<> struct abi<romancia::face_alignment>
	{
		...
		struct type : abi_unknown_object
		{
			...
			virtual std::int32_t G6_ABI_CALL get(abi_in_t<param_span<std::uint8_t>> bitmap, std::int32_t height, std::int32_t width, abi_in_t<exposing::param_vector<longinus::face_info>> faces, abi_out_t<param_vector< param_vector<std::uint8_t>>> result) noexcept = 0;
	...
	struct interface_vtable<Derived, romancia::face_alignment> : interface_vtable_base<Derived, romancia::face_alignment>
	{
		...
		virtual std::int32_t G6_ABI_CALL get(abi_in_t<param_span<std::uint8_t>> bitmap, std::int32_t height, std::int32_t width, abi_in_t<exposing::param_vector<longinus::face_info>> faces, abi_out_t<param_vector< param_vector<std::uint8_t>>> result) noexcept override
		{
			return abi_safe_call([&] { *result = detach_abi(this->self().get(create_from_abi<param_span<std::uint8_t>>(bitmap), height, width, create_from_abi<exposing::param_vector<longinus::face_info>>(faces))); });
		}
	...
	template<> struct abi_adapter<romancia::face_alignment>
	{
		...
		struct type : enable_self_abi_awareness<Derived, romancia::face_alignment>
		{
			...
			param_vector< param_vector<std::uint8_t>> get(param_span<std::uint8_t> bitmap, std::int32_t height, std::int32_t width, exposing::param_vector<longinus::face_info> faces) const
			{
