#pragma once
#ifndef _FACE_ALIGNMENT_NATIVE_HPP_
#define _FACE_ALIGNMENT_NATIVE_HPP_
#include <string>
...
namespace glasssix::romancia
{
	...
	public:
		...
		/// </summary>
		face_alignment_native();

		/// <summary>
		/// Creates an instance with a specified GPU core or the default CPU.
		/// </summary>
		/// <param name="device">The device ID; -1 for CPU or a non-negative number for a GPU core</param>
		face_alignment_native(int device);

		/// <summary>
		/// The copy constructor must be disabled in PImpl pattern.
		/// </summary>
		face_alignment_native(const face_alignment_native&) = delete;

		/// <summary>
		/// Destroys the instance.
		/// </summary>
		virtual ~face_alignment_native();

		/// <summary>
		/// The copy assignment operator must be disabled in PImpl pattern.
		/// </summary>
		face_alignment_native& operator=(const face_alignment_native&) = delete;
		...
		exposing::param_vector<std::uint8_t> get(exposing::param_span<std::uint8_t> gray_bitmap, std::int32_t height, std::int32_t width,
