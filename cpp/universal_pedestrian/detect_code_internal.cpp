...
namespace glasssix::universal_pedestrian
{
    ...
    public:
        impl(const exposing::param_string model_directory, int device = -1)
            : impl{get_model_params("universal_pedestrian", false), exposing::to_narrow_string(model_directory), device}
        {
        }

        impl(const std::vector<std::string> &phai, std::string model_directory, int device)
            : net_detect_(phai, model_directory + std::string("/universal_pedestrian.rknn"), device)
        // net_class_(phai,  model_directory + std::string("/universal_pedestrian_sim.rknn"), device)
        {
        }

        exposing::param_vector<universal_pedestrian::box_info> detect(const exposing::param_span<std::uint8_t> &bitmap, int channels, int height, int width, int roi_x, int roi_y, int roi_width, int roi_height, std::map<std::string, float> &param_map)
        {
            ...
            std::vector<universal_pedestrian::box_info_internal> result = universal_pedestrian_detect(cropped_image, con_thres, iou_thres);
            // std::cout<<"ok\n";
            // std::cout<<result[0].x1<<" "<<result[0].x2<<std::endl;

            auto results = exposing::make_param_vector<universal_pedestrian::box_info>();
    ...
    private:
        ...
        std::vector<universal_pedestrian::box_info_internal> universal_pedestrian_detect(cv::Mat &image, float con_thres, float iou_thres )
        {
    ...
    exposing::param_vector<universal_pedestrian::box_info> detect_code_internal::detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width,
