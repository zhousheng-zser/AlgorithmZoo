...
namespace glasssix
{
    int fxhog::static_Init(cv::Size _sz, int _cellSize)
    {
    ...
    int fxhog::compute(const cv::Mat &src, cv::Mat &dst, int _cellSize, float thres)
    {
        ...
        if (FXHOG_OK != getFeatureMaps(src))
        {
            return FXHOG_ERR_FEATUREMAP;
        }
        if (FXHOG_OK != normalizeAndTruncate(thres))
        {
            return FXHOG_ERR_NORM;
        }
        if (FXHOG_OK != PCAFeatureMaps())
        {
            return FXHOG_ERR_PCA;
        }
        dst = map;
        return FXHOG_OK;
    }
    ...
    int fxhog::getFeatureMaps(const cv::Mat &src)
    {
        ...
        cv::filter2D(src, dx, CV_32F, kernel_dx, cv::Point(-1, 0), 0.0, cv::BORDER_REFLECT);
        cv::filter2D(src, dy, CV_32F, kernel_dy, cv::Point(0, -1), 0.0, cv::BORDER_REFLECT);
        ...
        for (int i = 1; i < height - 1; ++i)
        {
            ...
            for (int j = 1; j < width - 1; ++j)
            {
                ...
                float y = pdy[j * channels];
                pr[j] = sqrtf(x * x + y * y);
                for (size_t c = 1; c < channels; c++)
                {
                    float tx = pdx[j * channels + c];
                    float ty = pdy[j * channels + c];
