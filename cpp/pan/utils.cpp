...
void utils::visualizeDeepsortResult(cv::Mat& image, std::vector<RESULT_DATA>& results, const std::vector<std::string>& classNames)
{
    for (auto ret : results) {
        int num = ret.first;
        auto x = ret.second(0);//x
        auto y = ret.second(1);//y
        //auto three = ret.second(2);//w
        //auto four = ret.second(3);//h

        cv::rectangle(image, cv::Rect(ret.second(0), ret.second(1), ret.second(2), ret.second(3)), cv::Scalar(229, 160, 21), 2);

        std::string label(std::to_string(ret.first));
        cv::Size size(cv::getTextSize(label, cv::FONT_ITALIC, 0.4, 2, 0));
        cv::rectangle(image,
            cv::Point(x, y - size.height * 1.3), cv::Point(x + size.width, y),
            cv::Scalar(229, 160, 21), -1);
...
//torch::Tensor utils::_make_grid(int nx = 20, int ny = 20) {
//    //yv, xv = torch.meshgrid([torch.arange(ny), torch.arange(nx)])
//    //return torch.stack((xv, yv), 2).view((1, 1, ny, nx, 2)).float()
//
//    auto ret = torch::meshgrid({ torch::arange(ny), torch::arange(nx) });
//    return torch::stack({ ret[1], ret[0] }, 2).view({ 1, 1, ny, nx, 2 }).toType(c10::kFloat);
//}