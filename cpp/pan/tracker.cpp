...
void tracker::update(const DETECTIONS& detections)
{
    ...
    for(Track track:tracks) {
        ...
        track.features = Eigen::Matrix<float, -1, 512, Eigen::RowMajor>(0,512);
    }
...
void tracker::_match(const DETECTIONS& detections, TRACHER_MATCHD& res)
{
    ...
    // ÐÙÑÀÀûÆ¥Åä 
    // cascade matching 
    // return match a:  matches¡¢ unmatched_tracks¡¢ unmatched_detections
    // match a:  existing match
    ...
    // unmatched tracks idxs & unmatched detectoins
    // retrun match b £º new tracker(old idx + new idx) + detections  
    // match b :  new match
    ...
    //unmatched_tracks:  match a + match b
