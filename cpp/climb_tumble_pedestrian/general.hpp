...
std::tuple<cv::Mat, float> preprocess_detection(cv::Mat& src, int& pad_h, int& pad_w, cv::Size input_shape = cv::Size(640, 640))
{
    ...
    {
        cv::resize(src, cut_image, cv::Size(std::round(src.cols * scale), std::round(src.rows * scale)), cv::INTER_LINEAR);

        pad_h = std::round( (input_shape.height - cut_image.rows) / 2.0-0.1  );
        pad_w = std::round( (input_shape.width - cut_image.cols) / 2.0 -0.1  );
