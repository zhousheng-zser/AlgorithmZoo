...
namespace glasssix::yolov5
{
    ...
    class yolov5m_net_internal::impl
    {
    public:
        impl(std::string_view yolov5m_racy_path, int device) 
            : impl{ hardcode::get_model_params("yolov5m"), yolov5m_racy_path, device }
        {
        }

        impl(std::string_view yolov5m_phai, std::string_view yolov5m_racy_path, int device)
            : device_{ device }, yolov5m_instance_{ std::string{yolov5m_phai}, std::string{yolov5m_racy_path}, device }
        {
        }

        impl(const std::vector<std::string>& yolov5m_phai, std::string_view yolov5m_racy_path,  int device)
            : device_{ device }, yolov5m_instance_{ yolov5m_phai, std::string{yolov5m_racy_path}, device }
        {
        }
...
#pragma omp section
        ...
        yolov5::result_info detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int order)
        {
            ...
            auto result = result_info(exposing::make_as_first<yolov5::result_info_impl>(result_internal));
    ...
    yolov5m_net_internal::yolov5m_net_internal(std::string_view yolov5m_racy_path, int device) 
        : impl_{ std::make_unique<impl>(yolov5m_racy_path, device) }
    {
    }

    yolov5m_net_internal::yolov5m_net_internal(std::string_view yolov5m_phai, std::string_view yolov5m_racy_path, int device)
        : impl_{ std::make_unique<impl>(yolov5m_phai, yolov5m_racy_path, device) }
    {
    }

    yolov5m_net_internal::yolov5m_net_internal(const std::vector<std::string>& yolov5m_phai, std::string_view yolov5m_racy_path, int device) 
        : impl_{ std::make_unique<impl>(yolov5m_phai, yolov5m_racy_path, device) }
    {
    }

    yolov5m_net_internal::yolov5m_net_internal()
    {
    }
    ...
    std::string yolov5m_net_internal::version()
    {
        return impl::version();
    }

    yolov5::result_info yolov5m_net_internal::detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int order) const
    {
