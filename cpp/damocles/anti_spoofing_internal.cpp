...
namespace glasssix::damocles
{
	namespace
	{
		...
		std::array<std::tuple<int, std::string, std::string>, 2> types =
		{
			{
				{0, "FASMV2", "FASMV2"},
				{1, "simple_G_MobileFaceNet", "simple_G_MobileFaceNet"}
			}
		};
	}
	...
	public:
		...
		impl(std::string_view models_directory, int model_type, int device) : impl{ model_type, device }
		{
			...
			spoofing_detect_instance_ = std::make_unique<rknnwrapper::rknn_wrapper>(hardcode::get_model_params(std::get<1>(*model_iter)), std::string(model_directory) + "/" + std::get<2>(*model_iter) + ".rknn", device);
			landmark65_ = std::make_unique<rknnwrapper::rknn_wrapper>(hardcode::get_model_params("pfld11_landmark65_simp"), std::string(model_directory) + "/pfld11_landmark65_simp.rknn", device);
#else
