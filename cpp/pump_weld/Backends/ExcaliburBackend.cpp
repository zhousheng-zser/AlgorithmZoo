...
class ExcaliburBackend : public InferBackend {
    std::unique_ptr<glasssix::excalibur::pipeline<float>> pipline;
...
public:
    ...
    int initModel(std::string arch, std::string weight, int device) override final {
        pipline = std::make_unique<glasssix::excalibur::pipeline<float>>(arch, weight, device);
        return 0;
    }
    ...
    int initModel(std::string model, int device) override final {
        ...
        pipline = std::make_unique<glasssix::excalibur::pipeline<float>>(phai, racy, device);
        return 0;
    }
