...
namespace glasssix
{
	namespace irisviel
	{
		...
		public:
			...
			virtual std::string_view key() const noexcept override
			{
				return record_->key;
			}
		...
		std::shared_ptr<database_record> database_record::create(int dimension, std::uint8_t* ptr)
		{
			...
			case 256:
				return std::make_shared<database_record_impl<512>>(*reinterpret_cast<basic_database_record<256>*>(ptr));
			case 512:
		...
		std::shared_ptr<database_record> database_record::create_ref(int dimension, std::uint8_t* ptr)
		{
			...
			case 256:
				return std::make_shared<database_record_ref_impl<512>>(reinterpret_cast<basic_database_record<256>*>(ptr));
			case 512:
		...
		{
			return key_equals(left.key().data(), right.key().data());
		}
	}
}
