...
namespace glasssix::irisviel
{
	struct feature_searcher
	{
		...
		virtual std::vector<std::vector<database_search_result>> search_vector(const std::vector<const float*>& query_data, std::optional<float> min_similarity, std::optional<std::uint32_t> top_k, bool result_has_feature) const = 0;
