...
{
	inline constexpr exposing::utf8_string_view call_detect_code_qualified_name{ u8"g6.onphone.detect_code" };
	...
	public:
		...
		exposing::param_vector<onphone::box_info> detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int roi_x, int roi_y, int roi_width, int roi_height,
			exposing::param_vector<head::box_info> head_info_list,
			const exposing::param_hash_map<exposing::param_string, float>& param_map_abi) const;

		exposing::param_vector<onphone::box_info> exdetect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int roi_x, int roi_y, int roi_width, int roi_height,
