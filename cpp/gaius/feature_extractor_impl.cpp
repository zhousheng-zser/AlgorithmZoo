...
namespace glasssix::gaius
{
	...
	void feature_extractor_impl::init(const exposing::param_string& racy_path, const exposing::param_string& mask_racy_path, std::int32_t device, bool use_int8)
	{
		impl_ = std::make_unique<feature_extractor_internal>(exposing::to_narrow_string(racy_path), exposing::to_narrow_string(mask_racy_path), device, use_int8);
	}
