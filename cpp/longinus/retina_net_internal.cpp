...
namespace glasssix::longinus
{
    ...
    class retina_net_internal::impl_yolo
    {
    public:
        impl_yolo(int model_type, float nms_threshold, int device) : model_type_{ model_type }, nms_threshold_{nms_threshold}, device_{ device } {}
        impl_yolo(std::string_view models_directory, int model_type, int detect_model_type, float nms_threshold = 0.4, int device = -1) : impl_yolo{model_type, nms_threshold, device}
        {
            ...
            {
            case 0:
                yolo_instance_ = std::make_unique<rknnwrapper::rknn_wrapper>(get_model_params(std::string("longinus")), std::string(models_directory) + "/longinusyolo320.rknn", device);
                break;
            case 1:
                yolo_instance_ = std::make_unique<rknnwrapper::rknn_wrapper>(get_model_params(std::string("longinus")), std::string(models_directory) + "/longinusyolo640.rknn", device);
                break;
            default:
...
#else
            yolo_instance_ = std::make_unique<excalibur::pipeline<float>>(get_model_params(std::string("longinus")), std::string(models_directory) + "/longinus.racy", device);
        ...
        exposing::param_vector<face_info> detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int min_size, float threshold, int order, bool do_attributing)
        {
            ...
            std::tie(blob, ratio) = preprocess_detection(cache_temp,pad_h,pad_w,new_shape);
            ...
            auto  network_result = 
                            yolo_instance_->forward(blob.data, { 1, blob.rows, blob.cols,blob.channels() }, RKNN_TENSOR_NHWC);
            ...
            std::shared_ptr<glasssix::memory::tensor<float>> tensor_stride8
                    (new memory::tensor<float>(std::vector<int>{3,img_size/8,img_size/8,6}, -1, memory::NCHW));
            std::shared_ptr<glasssix::memory::tensor<float>> tensor_stride16
                    (new memory::tensor<float>(std::vector<int>{3,img_size/16,img_size/16,6}, -1, memory::NCHW));
            std::shared_ptr<glasssix::memory::tensor<float>> tensor_stride32
                    (new memory::tensor<float>(std::vector<int>{3,img_size/32,img_size/32,6}, -1, memory::NCHW));
...
#if defined(USE_RKNNAPI) || defined(USE_RKNN2API)
        void tracking_landmark(cv::Mat& face, face_info_internal& trackfaceinfo, int offset_x, int offset_y)//1.face crop  2.souface  
        {
            int width = face.cols;
            ...
            std::vector<int> shape_215{ 1, 14, 1, 1 };
...
#if defined(USE_RKNNAPI) || defined(USE_RKNN2API)
//#if 0
        std::unique_ptr<rknnwrapper::rknn_wrapper> yolo_instance_;
        std::unique_ptr<rknnwrapper::rknn_wrapper> tracker_;
#else
        std::unique_ptr<glasssix::excalibur::pipeline<float>> yolo_instance_;
...
#if defined(USE_RKNNAPI) || defined(USE_RKNN2API)
        exposing::param_vector<face_info> detect_yolo(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int min_size, float threshold, int order, bool do_attributing)
        {
            if (bitmap.empty())
            {
                throw exposing::abi_invalid_argument("current frame is empty");
...
//#if 0
    ...
    retina_net_internal::retina_net_internal(std::string_view models_directory, int model_type, int  instance ,float nms_threshold, int device) 
    {
        instance_ = instance;

        if(instance_==0)
        {
            impl_ = std::make_unique<impl>(models_directory, model_type, nms_threshold, device);
        }
        else
        {
            impl_yolo_ = std::make_unique<impl_yolo>(models_directory, model_type, nms_threshold, device);
        }
    ...
    exposing::param_vector<face_info> retina_net_internal::detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int min_size, float threshold, int order, bool do_attributing)
    {
        ...
        {
            return impl_yolo_->detect(bitmap, channels, height, width, min_size, threshold, order, do_attributing);
        }
    }
    ...
    face_info retina_net_internal::single_trace(face_info face, exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int order)
    {
        ...
        {
            return impl_yolo_->single_trace(face, bitmap, channels, height, width, order);
        }
        ...
        float scale, std::int32_t order) const
    {
        ...
        {
            return impl_yolo_->center_scale_align(bitmap, channels, height, width, scale, order);
        }
