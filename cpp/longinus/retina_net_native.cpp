...
namespace glasssix::longinus
{
	...
	retina_net_native::retina_net_native(std::string phai_path, std::string racy_path, float nms_threshold, int device)
	...
	retina_net_native::~retina_net_native()
	{
		delete allocator;
	}

	std::vector<anchor_box> retina_net_native::bbox_pred(std::vector<anchor_box> anchors, std::vector<std::vector<float>> regress)
	{
		//"""
	...
	anchor_box retina_net_native::bbox_pred(anchor_box anchor, std::vector<float> regress)
	{
		anchor_box rect;
	...
	std::vector<face_pts> retina_net_native::landmark_pred(std::vector<anchor_box> anchors, std::vector<face_pts> facepts)
	{
	...
	face_pts retina_net_native::landmark_pred(anchor_box anchor, face_pts facePt)
	{
		face_pts pt;
		float width = anchor.w;
		float height = anchor.h;
	...
	bool retina_net_native::compare_bbox(const face_info_internal& a, const face_info_internal& b)
	{
		return a.score > b.score;
	}

	std::vector<face_info_internal> retina_net_native::nms(std::vector<face_info_internal>& bboxes, float threshold)
	{
	...
	std::string retina_net_native::version()
	{
		return "1.0.0";
	}


	std::vector<face_info_internal> retina_net_native::detect(const unsigned char* bitmap, int channels, int height, int width, int min_size, float threshold, int order)
	{
		if (!bitmap)
		{
