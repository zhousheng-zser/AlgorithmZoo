...
class BMNNBackend : public InferBackend {
	std::unique_ptr<ONNXRTPipline> pipline;
...
public:
	...
	const std::string getVersion() override final {
		//return pipline->version(); // not implement
		return "sophone_bmnn";
	}
	...
	int initModel(std::string arch, std::string weight, int device) override final {
		if (arch == weight) {
			...
			pipline = std::make_unique<ONNXRTPipline>(target_model, device);
			return 0;
		}
		else {
			return 1;
	...
	SupportHandNormaliztion handset_possible_normalization(std::array<float, 3> means, std::array<float, 3> stands) {
		...
		pipline->setStdMean(img_mean, img_stand);
