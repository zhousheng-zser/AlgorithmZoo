...
class pp_onphone_4b : public Postprocessing
{
    ...
    static std::unordered_map<std::string, std::shared_ptr<memory::tensor<float>>> onphone4b_concat(std::unordered_map<std::string, std::shared_ptr<memory::tensor<float>>>& input_tensor_map)
    {
        auto det_rst_vec = sort_yolo_rst(input_tensor_map);

        TensorSptr concat_tensor_ptr = yolov8_complement(det_rst_vec);

        const int _UINTLINE_NUM = concat_tensor_ptr->width();
        const int _LINES = concat_tensor_ptr->height();
        auto concat_tensor_data = concat_tensor_ptr->cpu_data();

        //score_array
        std::shared_ptr<glasssix::memory::tensor<float>> output_score(new memory::tensor<float>(std::vector<int>{1, 1, _LINES, 1}, -1, memory::NCHW));
        std::shared_ptr<glasssix::memory::tensor<float>> output_locat(new memory::tensor<float>(std::vector<int>{1, 1, _LINES, _UINTLINE_NUM - 1}, -1, memory::NCHW));
        float* output_score_data = output_score->mutable_cpu_data();
        float* output_locat_data = output_locat->mutable_cpu_data();
        for (int i = 0; i < _LINES; i++)// 8400 = 6400 + 1600 + 400 (80,40,20); 33600 = 25600 + 6400 + 1600
        {
            for (int j = 0; j < 4; j++) {
        ...
        npy::SAVE_TENSOR_TO_NUMPY(concat_tensor_ptr, "D:/playphone_4bo.npy");
...
public:
    virtual const std::map<std::string, postprocessing_function> parser_postprocessing_dump() const override
    {
        ...
        pp_map["onphone_4b"] = &onphone4b_concat;
        return pp_map;
    }
};

REGISTE_POSTPROCESSING(pp_onphone_4b)