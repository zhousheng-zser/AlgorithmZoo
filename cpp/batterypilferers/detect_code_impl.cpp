...
namespace glasssix::batterypilferers
{
	...
	// exposing::param_vector<batterypilferers::box_info> detect_code_impl::detect(exposing::param_span<std::uint8_t> bitmap, int channels,
	// 	int height, int width, int roi_x, int roi_y, int roi_width, int roi_height, const exposing::param_hash_map<exposing::param_string, float>& param_map_abi) const
	// {
	// 	if (!impl_)
	// 		throw exposing::abi_invalid_operation(u8"batterypilferers internal object not initialized");
