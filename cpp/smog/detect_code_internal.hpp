#ifndef __SOMG_DETECT_CODE_INTERNAL_HPP__
#define __SOMG_DETECT_CODE_INTERNAL_HPP__
...
namespace glasssix::smog
{
	...
	struct NMSBox {
		...
		NMSBox(float cx, float cy, float w, float h, float the_score) {
			xmin = cx - w / 2;
			xmax = cx + w / 2;
			ymin = cy - h / 2;
			ymax = cy + h / 2;
    ...
    public:
        ...
        exposing::param_vector<smog::box_info> detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int roi_x, int roi_y, int roi_width, int roi_height, std::map<std::string, float>& param_map) const;
