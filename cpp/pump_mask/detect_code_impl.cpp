...
namespace glasssix::pump_mask
{
	...
	exposing::param_vector<pump_mask::box_info> detect_code_impl::detect(exposing::param_span<std::uint8_t> bitmap, int channels,
		int height, int width, const exposing::param_hash_map<exposing::param_string, float>& param_map_abi) const
	{
		if (!impl_)
			throw exposing::abi_invalid_operation(u8"pump_mask internal object not initialized");
