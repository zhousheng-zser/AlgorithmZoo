...
namespace glasssix::workcloth
{
	...
	exposing::param_vector<workcloth::box_info> classify_code_impl::detect(exposing::param_span<std::uint8_t> bitmap, int channels, int height, int width, int roi_x, int roi_y, int roi_width, int roi_height, int strategy,
		const exposing::param_hash_map<exposing::param_string, float>& param_map_abi) const
	{
		...
		return impl_->detect(std::move(bitmap), channels, height, width, roi_x, roi_y, roi_width, roi_height, strategy, param_map);
	}
