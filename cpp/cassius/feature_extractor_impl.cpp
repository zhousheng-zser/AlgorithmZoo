...
namespace glasssix::cassius
{
    ...
    void feature_extractor_impl::init(std::int32_t model_type, const exposing::param_string &racy_path, std::int32_t device, bool use_int8)
    {
        impl_ = std::make_unique<feature_extractor_internal>(model_type, exposing::to_narrow_string(racy_path), device, use_int8);
    }
