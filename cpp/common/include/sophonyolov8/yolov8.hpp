#ifndef YOLOV8_H
#define YOLOV8_H

#include <iostream>
#include <vector>
#include "opencv2/opencv.hpp"
#include "../BMNNWrapper/bmnn_utils.h"
#include "utils.hpp"
#include "bm_wrapper.hpp"
// Define USE_OPENCV for enabling OPENCV related funtions in bm_wrapper.hpp
#define USE_OPENCV 1
#define DEBUG 0


enum class PostureType { K17 = 0, K12 = 1 };

struct sophonkey_point
{
    float x;
    float y;
    float score;
    sophonkey_point(float x_, float y_, float score_) :x(x_), y(y_), score(score_)
    {}
};

struct YoloV8Box {
    float x1, y1, x2, y2;

    float score;
    int class_id;
    std::vector<sophonkey_point> key_points;
};


using YoloV8BoxVec = std::vector<YoloV8Box>;

class YoloV8 {
    protected:
        std::shared_ptr<BMNNContext> m_bmContext;
        std::shared_ptr<BMNNNetwork> m_bmNetwork;

        std::vector<bm_image> m_resized_imgs;
        std::vector<bm_image> m_converto_imgs;

        //configuration
        float m_confThreshold= 0.25;
        float m_nmsThreshold = 0.7;

        std::vector<std::string> m_class_names;
        int m_class_num = 80; // default is coco names
        int mask_num = 0;
        int m_net_h, m_net_w;
        int max_batch;
        int output_num;
        int min_dim;
        int max_det=300;
        int max_wh=7680; // (pixels) maximum box width and height
        bmcv_convert_to_attr converto_attr;

        TimeStamp *m_ts;

    public:   
        int pre_process(const bm_image& images);
        virtual int post_process(const bm_image& images, YoloV8BoxVec& boxes,float conf=0.5,float nms_thresh=0.6);
        static float get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool *alignWidth);
        void xywh2xyxy(YoloV8BoxVec& xyxyboxes, std::vector<std::vector<float>> box);
        static bool YoloV8Box_cmp(YoloV8Box a, YoloV8Box b);
        void NMS(YoloV8BoxVec &dets, float nmsConfidence);
        void clip_boxes(YoloV8BoxVec& yolobox_vec, int src_w, int src_h);

   
        YoloV8(std::shared_ptr<BMNNContext> context);
        virtual ~YoloV8();
        // void set_confthreshold(float conf,float nms_thresh=0.6 );
        int Init(float confThresh=0.5, float nmsThresh=0.5);
        virtual void setyolov8_type(PostureType yolov8type);
        void enableProfile(TimeStamp *ts);
        int batch_size();
        int Detect(const bm_image& images, YoloV8BoxVec& boxes,float conf,float nms_thresh=0.6);
        void draw_bmcv(bm_handle_t &handle, int classId, float conf, int left, int top, int right, int bottom, bm_image& frame, bool put_text_flag=false);
};

#endif //!YOLOV8_H
