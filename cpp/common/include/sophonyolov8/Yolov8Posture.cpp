...
#define USE_MULTICLASS_NMS 1
  ...
  Yolov8Posture::Yolov8Posture(std::shared_ptr<BMNNContext> context)
    : YoloV8(context), posturetype(PostureType::K17)
    {
        std::cout<<"Yolov8Posture construct \n";
        PostureKeyinfo = {{PostureType::K17, 17}, {PostureType::K12, 12}};
    }

    void Yolov8Posture::setyolov8_type(PostureType yolov8type)
    {
        this->posturetype = yolov8type;
    }

    int Yolov8Posture::post_process(const bm_image& image, YoloV8BoxVec& detected_boxes,float conf,float nms_thresh) {
    YoloV8BoxVec yolobox_vec;
    std::vector<cv::Rect> bbox_vec;
    std::vector<std::shared_ptr<BMNNTensor>> outputTensors(output_num);
    // std::cout<<"Yolov8Posture post_process \n";
