...
// void YoloV8::set_confthreshold(float conf,float nms_thresh=0.6 )
// {
...
int YoloV8::Detect(const bm_image& input_images, YoloV8BoxVec& boxes,float conf,float nms_thresh) {
    int ret = 0;
    // 3. preprocess
    ...
    ret = post_process(input_images, boxes,conf, nms_thresh);
    CV_Assert(ret == 0);
    // LOG_TS(m_ts, "yolov8 postprocess");
    return ret;
}

int YoloV8::pre_process(const bm_image& image) {
    ...
    for (int i = 0; i < 1; ++i) {
        ...
        float ratio = get_aspect_scaled_ratio(image.width, image.height, m_net_w, m_net_h, &isAlignWidth);
        ...
        if (isAlignWidth) {
            padding_attr.dst_crop_h = image.height * ratio;
            ...
            padding_attr.dst_crop_w = image.width * ratio;
...
#else
        auto ret = bmcv_image_vpp_convert(m_bmContext->handle(), 1, image, &m_resized_imgs[i]);
#endif
        assert(BM_SUCCESS == ret);
        if (need_copy)
